      ...
      <div class="container">
        ...
        <div class="dable_lsit">
          ...
          <table cellSpacing="0px" cellpadding="0">
            ...
            <tbody>
            <tr v-for="(item,index) in tableListData.serviceDiscounts">
              <td>
                {{index+1}}
              </td>
              <td>
                {{item.discountName|placeholder}}
                <input  type="text" v-model="item.discountLink" class="link">
              </td>
              <td>
              ...
              <td>
                <span>
                    <el-button  type="text" @click="copy(index)">
                      <i></i>
                        复制链接
                    </el-button>
                </span>
                <span class="details_icon" v-show="$isButtonObj('bn-master-account-details')"><el-button type="text" @click="edit(item,index)"><i></i>详情</el-button></span>
                <span class="details_icon" v-show="$isButtonObj('bn-master-account-details')&&item.discountMode==='CC'">
                  <a 
                  @click="getDownLaod(item,$event)" 
                  class="downlaod" 
...
//      <!--弹窗功能start-->
        add(){
          ...
          this.isAdd.data=false;
        },
        edit(item,index){
          this.isAdd.data=item;
          this.isAdd.title="编辑优惠券";
          this.isAdd.isShow=true
        },
//      <!--弹窗功能end-->
        derive(){
